...
To address the limitations of purely extractive or purely abstractive approaches on such long documents, I propose a \textbf{Hybrid Pipeline}. This architecture uses an Extractive model as a "content selector" to filter the document, followed by an Abstractive model acting as a "rewriter" to synthesize the selection.
...
For the extraction phase, I employ \textbf{MatchSum} (Matching Summarization), which represents a shift from sentence-level scoring to document-level matching.
\begin{itemize}
    \item \textbf{Base Architecture (BERT):} I use \texttt{bert-base-romanian-cased-v1} (\textbf{$\approx$ 110M parameters}), a Transformer encoder pre-trained on a large corpus of Romanian text. BERT uses a mechanism called "Self-Attention" to understand the context of each word relative to every other word.
...
For generation, I use \textbf{mT5-small (Multilingual Text-to-Text Transfer Transformer)} which has roughly \textbf{300M parameters}.
\begin{itemize}
    ...
    \item \textbf{Fine-tuning:} I fine-tuned the \texttt{google/mt5-small} checkpoint specifically on my Parliamentary dataset to adapt its generation style to the formal, legislative language of the Romanian Chamber of Deputies.
\end{itemize}
...
I constructed a custom dataset by scraping the official \textit{Camera Deputaților} website.
\begin{itemize}
    \item \textbf{Source:} \texttt{cdep.ro} (Debates section).
    \item \textbf{Extraction:} I extracted the full transcript (Source) and the "SUMAR" section (Target/Ground Truth).
    ...
    \item \textbf{Splits:} 
    \begin{itemize}
        \item \textbf{Training Data:} I utilized sessions with IDs \textbf{8000 to 9000}, corresponding roughly to the \textbf{2019-2022} legislative period.
        \item \textbf{Evaluation Data:} I evaluated on a held-out set of IDs \textbf{7800 to 7950}, primarily from the \textbf{2017-2018} sessions.
    \end{itemize}
\end{itemize}
...
I implemented a robust scraper (\texttt{CDEPScraper}) that handles legacy SSL protocols (SECLEVEL=1) required to access the government server. It iterates through session IDs, extracting the raw HTML content.
\\
...
\begin{enumerate}
    ...
    \item \textbf{Spacy Segmentation:} I use \texttt{ro\_core\_news\_sm} to split the text into sentences. This is non-trivial because of abbreviations (e.g., "Art. 5", "O.U.G. nr. 10"). Spacy's dependency parser helps correctly identify sentence boundaries where simple rule-based splitting fails.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item I tokenize every sentence individually ($S_i$).
        \item I compare each $S_i$ against the dense vector representation of the document's context ($D$).
    ...
    \end{itemize}
    \item \textbf{Selection Heuristic & Parameter Tuning:} I empirically determined a selection ratio of $r=0.15$ with strict bounds:
    \[ k = \max(20, \min(80, N \times 0.15)) \]
    The rationale behind these values is threefold:
    \begin{itemize}
        \item \textbf{Why 15\%?} There is a natural trade-off between ROUGE scores and summary conciseness. My experiments demonstrated that increasing the ratio from 5\% to 10\% to 15\% yielded consistent gains in ROUGE (higher recall of key facts). However, beyond this point, the gain diminishes while the noise drastically increases.
        ...
        \item \textbf{Why Min=20?} For extremely short debates, a percentage-based cut might yield only 1-2 sentences, lacking the context required for a coherent narrative. I enforce a minimum floor to ensure the generator always has enough material to work with.
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Regex Post-Processing:} Before generation, I strip specific parliamentary boilerplate that might confuse the model. This is handled in \\ \texttt{src/processing/post\_processing.py}:
    ...
    \item \textbf{Generation Override:} I explicitly override the default generation configuration to prevent premature stopping:
    \begin{lstlisting}[language=Python]
    gen = pipeline(
...
I evaluated my models in a test set of \textbf{64 meetings}. I utilized the \textbf{ROUGE} metric (Recall-Oriented Understudy for Gisting Evaluation) to measure overlap with the official human-written summaries.
...
To quantitatively assess the quality of my summaries, I utilize the \textbf{ROUGE} (Recall-Oriented Understudy for Gisting Evaluation) metric, which is the standard de-facto metric for NLP summarization tasks. ROUGE measures the overlap between the generated summary ($S$) and the reference human summary ($R$).
...
\begin{itemize}
    ...
    \item \textbf{Performance Context:} My Hybrid model's score of \textbf{0.3439} is highly competitive for the Romanian language, especially given the difficulty of the parliamentary domain (long-context). It aligns with recent benchmarks on Romanian GPT-2 based summarization.
